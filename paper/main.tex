...
motile cilia~\cite{Olstad2019CiliaryDevelopment} (\Cref{fig:figure1}d, pink markers).
\begin{figure}%[hbt!]
    \centering
...
On the ciliated regions of the ventricle walls $\Gc$ (\Cref{fig:figure1}d),
...
and the sign of the vector $\rr=\pm(1, 0, 1)$ varies depending on the cilia region (see~\Cref{subsec:appendixA1}).
...
$\Tilde{p}(t)=-A\sin{\omega t}$ on anterior and posterior parts, respectively, of the boundary $\Gp$~(\Cref{fig:figure1}d, blue markers).
...
and out of the domain in a manner that ensures conservation of mass (see~\Cref{subsec:adv_diff_eq_weak_form} for details).
...
\begin{table}[h!]
    ...
    \bottomrule
    \end{tabular}
    \caption{Physical parameters of the flow and transport models.
    (*) Calibrated based on experimental data~\cite{Olstad2019CiliaryDevelopment}.
    (\textdagger) Estimated based on experimental
    data~\cite{GuraSadovsky2017MeasurementExpansion, Swaminathan1997PhotobleachingDiffusion, Potma2001ReducedCells}.}
...
An illustration of the ROIs is provided in \Cref{fig:fig2}e. The dynamics of ROI 1 follow from imposing~\Cref{eq:photoconversion_curve}.
