...
\keywords{cerebrospinal fluid, brain ventricles, transport, finite element methods}
...
From the imaging data, we constructed a computational mesh of the ventricles (\Crefpanels{fig:fig1}{d}).
...
motile cilia (\Crefpanels{fig:fig1}{d}, magenta markers)~\cite{Olstad2019CiliaryDevelopment}.
\begin{figure}%[hbt!]
    \centering
    ...
    a time 25\% into the cardiac cycle. %t=0.112613 s
    \textbf{h.} Streamlines simulated with the baseline flow model at
    a time 50\% into the cardiac cycle. %t=0.225225 s
    \textbf{i.} Streamlines simulated with the baseline flow model at
    a time 75\% into the cardiac cycle. %t=0.315315 s
...
On the ciliated regions of the ventricle walls $\Gc$ (\Crefpanels{fig:fig1}{d}, magenta markers),
...
For the exosomes, we calculate $D$ using~\Cref{eq:D_stokes_einstein}
...
available~\cite{zenodo-link-to-all-code}\fixme{Add repo link.}. 
...
the repository of this article~\cite{zenodo-link-to-all-code}\fixme{Add repo link.}.
...
To recover the flow fields shown in~\Crefpanels{fig:fig1}{b, c}, we carried out
...
covering a distance of at least 8.8 \textmu m, were plotted in~\Crefpanels{fig:fig1}{b, c}.
...
We model the compartmentalized and pulsatile CSF flow observed in vivo~(\Crefpanels{fig:fig1}{b, c})
...
ventricles~(~\Crefpanels{fig:fig1}{e}). The highest velocities ($26.5$ \textmu m/s)
...
flow is pulsatile as a result of the sinusoidal pressure gradient (~\Crefpanels{fig:fig1}{f, g}),
...
and directional flow in the posterior ventricle (~\Crefpanels{fig:fig1}{h--j},
...
(~\Crefpanels{fig:fig2}{a}). We consider the spread of
...
(\Crefpanels{fig:fig2}{b}). The vortex structures of the velocity field
...
ventricular geometry (\Crefpanels{fig:fig2}{c, d}). Notably, the
...
(\Crefpanels{fig:fig2}{e--g}) and
the times $\hat{t}$ when the mean concentrations $\cbar_i$ first exceed
a threshold value $\hat{c}$ (\Crefpanels{fig:fig2}{h})
in 6 ROIs (\Crefpanels{fig:fig2}{a}).
...
over time (\Crefpanels{fig:fig3}{a},
$F_0$ being the average fluorescence at baseline). 

We obtained very similar kinetics curves (\Crefpanels{fig:fig3}{a, b})
and time-to-threshold values (\Crefpanels{fig:fig3}{c, d}) when comparing our
experiments (\Crefpanels{fig:fig3}{a, c}) with
...
By plotting the means of $\Delta F$ (\Crefpanels{fig:fig4}{a})
and the time to threshold
across the control and mutant cohorts (\Crefpanels{fig:fig4}{b}),
...
mutants compared to controls (\Crefpanels{fig:fig4}{b}). 
...
(both cilia and cardiac motion) (\Crefpanels{fig:fig4}{c}).
...
exhibit changes in the dynamics of $\cbar$ (\Crefpanels{fig:fig5}{c}).
...
mean concentration $\cbar$ with time (\Crefpanels{fig:fig5}{c}).
...
and the combination of the above three modifications (\Crefpanels{fig:fig6}{a}).
...
\Cref{fig:fig4}, as the model and the fish have different duct diameters.
...
\begin{figure}[H]
    ...
    in the 6 ROIs. 
    \mk{MK: How do the different geoemtries compare in terms of volume,
...
towards the wall~(\Crefpanels{fig:fig1}c).
...
in advection-dominated regions. Using conventional Taylor-Hood elements for the Stokes equations,
...
method~\cite{Franca1992StabilizedModel} instead of the current DG method.
...
\lyng{HH: Yes, we should expect 2 there? Maybe be more specific in the comment:
"Using polynomial degree $k=1$, we observe linear convergence in both the velocity $H^1$ error
and the pressure $L^2$ error. For the $L^2$ error of the velocity, we observe a suboptimal
convergence order of 1.5"}
...
mammalian ependymal cells -- will generate much stronger velocities.
Hence advection may be more dominant in those situations.
However, in large brains such as the human brain, cilia beating will
only contribute to flow in localized regions of very high proximity to the ventricular walls,
...
only contributes to near-wall flow dynamics, but flow in the entire ventricular system.
...
The data and implementations presented in this paper are licensed under \fixme{LICENSE}
and openly avalaible at \fixme{REPO-LINK}.
...
\begin{align*}
    ...
    \mathbf{H}^1(\mathrm{div}; \Omega)
    &= \bigg\{\uu \ : \ \uu\in\mathbf{L}^2(\Omega), \nabla\cdot\uu\in L^2(\Omega)\bigg\}.
\end{align*}
...
\begin{equation*}
    \VV =  \bigg\{\vv \ : \ \vv\in\mathbf{H}^1(\mathrm{div}; \Omega),
...
well defined gradient (and in turn $\beps$). However, for the derivations of
...
condition~\Cref{eq:modelA_slip_BC_impermeability}.
Consequently, the test function counterpart $\vv\cdot\nn$ vanishes
on the boundary, and the term~\Cref{eq:boundary_integral_BC_normal}
...
the boundary $\Gamma$, the vector $(\uu, p)=(\mathbf{0}, C)$ for any real
constant $C$ satisfies the equations.
The system~\Cref{eq:stokes_weak_modelA} is thus singular with a one-dimensional nullspace
of constant pressures.
Consequently, assembly of our discretization of the  equations~\Cref{eq:stokes_weak_modelA}
into a linear equation system $\mathbf{A}\xx=\mathbf{b}$ yields a singular matrix $\mathbf{A}$.
...
\begin{table}[H]
    ...
    \begin{tabular}{lcc}
        ...
        & $\ \ \ (1, 0, 1)$
        & $\lambda(\xx) = \frac{1}{2}\left(1 - \frac{x-x_{0, v}}{x_{e, v} - x_{0, v}}\right)$ \\
        \bottomrule
    \end{tabular}
\end{table}
\endgroup
...
anterior ventricle and one on the posterior ventricle~(\Cref{fig:fig1}\fixme{d}, blue markers).
...
condition~\Cref{eq:pressure_BC_modelC} is applied as discussed in the section
...
formulation~\Cref{eq:abstract_weak_form_modelC,eq:stokes_bilinear_form,eq:stokes_skew_form,eq:stokes_linear_form}
...
where $\mathcal{F} = \{\mathcal{F}_k : k\in K\}$ is the set of all interior facets
$\mathcal{F}_k$ of the computational mesh. The parameter $\gamma>0$ is
introduced to penalize jumps in the tangential components of the velocity
across facets, wheras $h$ is the cell diameter of a mesh cell,
defined as the maximal distance between two points in a cell.
The semi-discrete weak formulation of the Stokes equations reads:
...
discontinuous linear Lagrange polynomial basis functions,
allowing for discontinuities in the discrete solution across element edges.
To achieve a stable discretization with these elements, we add the stabilization
\begin{equation}
    \begin{aligned}
        a_{\mathrm{stab}}(c_h, \phi_h)
        = &\sum_{k\in K}\intF{D\frac{\alpha}{\avg{h}}\jump{c_h}\cdot\jump{\phi_h}}
        +\sum_{k\in K}\intF{\jump{\phi_h}\cdot\jump{c_h \hat{u}_h}}  \\
...
\end{equation*}
is used to upwind the velocity $\uu_h$, with the velocity upwinded with respect to a
facet $\mathcal{F}_k$. The stabilization parameter $\alpha>0$ must be chosen
large enough to achieve stability. The average and jump operators used
...
amplitude $A$. The choice of $A=0.0015$ Pa was used since: (i) it yields a
