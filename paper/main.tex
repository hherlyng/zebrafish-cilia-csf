...
These simulation studies have provided valuable insights on how cilia \fixme{pump fluids}
and may contribute to tissue physiology.
...
From the imaging data, we constructed a computational mesh of the ventricles (\Cref{fig:fig1}\fixme{d}).
...
motile cilia~\cite{Olstad2019CiliaryDevelopment} (\Cref{fig:fig1}\fixme{d}, magenta markers).
\begin{figure}%[hbt!]
    \centering
    ...
    a time 70\% into the cardiac cycle. %t=0.315315 s
    }
    \label{fig:fig1}
\end{figure}
...
within the brain ventricles (\Cref{fig:fig1}\fixme{a--c}).
...
On the ciliated regions of the ventricle walls $\Gc$ (\Cref{fig:fig1}\fixme{d}, magenta markers),
...
boundary $\Gp$~(\Cref{fig:fig1}\fixme{d}, blue markers).
...
To recover the flow fields shown in~\Cref{fig:fig1}\fixme{b, c}, we carried out
...
covering a distance of at least 8.8 \textmu m, were plotted in~\Cref{fig:fig1}\fixme{b, c}.
...
We model the compartmentalized and pulsatile CSF flow observed \emph{in vivo}~(\Cref{fig:fig1}\fixme{b--c})
...
ventricles~(\Cref{fig:fig1}\fixme{e}). The highest velocities ($26.5$ \textmu m/s)
...
flow is pulsatile as a result of the sinusoidal pressure gradient (\Cref{fig:fig1}\fixme{g}),
...
and directional flow in the posterior ventricle (\Cref{fig:fig1}\fixme{e},
...
CSF flow model reproduces the flow features observed \emph{in vivo}~\cite{Olstad2019CiliaryDevelopment}.
...
The final values of $\cbar$ in our simulations 0.76, 0.72, 0.82, 0.56 and 0.35 (ROIs 2--6, respectively)
were generally higher than the experimental values 0.63, 0.71, 0.67, 0.29 and 0.18 for $\Delta F$.
...
\begin{figure}[H]
    ...
    \caption{\fixme{Create common legend.}
    \textbf{a.} Relative change in fluorescence intensity $\Delta F$ over time
...
in control and motile-cilia mutant zebrafish (\emph{smh}).
...
\Cref{fig:fig4}, which have different duct diameters.
