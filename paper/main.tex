...
\author[2]{Inyoung Jeong} % OK?
\author[1,3,+]{Marie E.~Rognes} % OK order?
\author[2,+,*]{Nathalie Jurisch-Yaksi}
\affil[1]{Department of Numerical Analysis and Scientific Computing,
Simula Research Laboratory, Oslo, Norway}
\affil[2]{Department of Clinical and Molecular Medicine,
Norwegian University of Science and Technology, Trondheim, Norway}
\affil[3]{K.~G.~Jebsen Center for Brain Fluid Research}
\affil[*]{\href{mailto:hherlyng@simula.no}{hherlyng@simula.no},
\href{mailto:nathalie.jurisch-yaksi@ntnu.no}{nathalie.jurisch-yaksi@ntnu.no}}
...
From the imaging data, we constructed a computational mesh of the ventricles (\Cref{fig:fig1}d).
...
motile cilia~\cite{Olstad2019CiliaryDevelopment} (\Cref{fig:fig1}d, magenta markers).
\begin{figure}%[hbt!]
    \centering
...
within the brain ventricles (\Cref{fig:fig1}a--c).
...
On the ciliated regions of the ventricle walls $\Gc$ (\Cref{fig:fig1}d, magenta markers),
...
boundary $\Gp$~(\Cref{fig:fig1}d, blue markers).
...
In this case, we first solve the steady-state Stokes equations~\eqref{eq:stokes_eqs}
...
To recover the flow fields shown in~\Cref{fig:fig1}b, c, we carried out
...
covering a distance of at least 8.8 \textmu m, were plotted in~\Cref{fig:fig1}b, c.
...
We model the compartmentalized and pulsatile CSF flow observed \emph{in vivo}~(\Cref{fig:fig1}b, c)
...
ventricles~(\Cref{fig:fig1}e). The highest velocities ($26.5$ \textmu m/s)
...
flow is pulsatile as a result of the sinusoidal pressure gradient (\Cref{fig:fig1}f, g),
...
and directional flow in the posterior ventricle (\Cref{fig:fig1}h--j,
...
\emph{in vivo}~\cite{Olstad2019CiliaryDevelopment}.
...
(\Cref{fig:fig2}a). We consider the spread of
...
(\Cref{fig:fig2}b). The vortex structures of the velocity field
...
ventricular geometry (\Cref{fig:fig2}c, d). Notably, the
...
(\Cref{fig:fig2}e--g) and
the times $\hat{t}$ when the mean concentrations $\cbar_i$ first exceed
a threshold value $\hat{c}$ (\Cref{fig:fig2}h)
in 6 ROIs (\Cref{fig:fig2}a).
...
over time (\Cref{fig:fig3}a,
$F_0$ being the average fluorescence at baseline). 

We obtained very similar kinetics curves (\Cref{fig:fig3}a, b)
and time-to-threshold values (\Cref{fig:fig3}c, d) when comparing our
experiments (\Cref{fig:fig3}a, c) with
simulation (\Cref{fig:fig3}b, d).
...
By plotting the means of $\Delta F$ (\Cref{fig:fig4}a)
and the time to threshold
across the control and mutant cohorts (\Cref{fig:fig4}b),
...
mutants compared to controls (\Cref{fig:fig4}b). 
...
(both cilia and cardiac motion) (\Cref{fig:fig4}c).
...
of the diencephalic (middle) ventricle (\Cref{fig:fig5}a).
...
exhibit changes in the dynamics of $\cbar$ (\Cref{fig:fig5}b).
...
mean concentration $\cbar$ with time (\Cref{fig:fig5}b).
...
\begin{figure}[H]
    ...
    \textbf{b.} Mean concentration $\cbar(t)$ for the six regions of interest (ROIs) in the three 
...
and the combination of the above three modifications (\Cref{fig:fig6}a).
...
that these cilia generate the strongest flow field \emph{in vivo}~\cite{Olstad2019CiliaryDevelopment} and in our model.
...
towards the wall~(\Cref{fig:fig1}\fixme{c}).
