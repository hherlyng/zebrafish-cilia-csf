...
\author[\fixme{X}]{Inyoung Jeong} % OK?
...
covering a distance of at least 8.8 $\mu$m, were plotted in~\Cref{fig:fig1}\fixme{b, c}.
...
(\Cref{fig:fig2}\fixme{b--d}) and
the times $\hat{t}$ when the mean concentrations $\cbar_i$ first exceed
a threshold value $\hat{c}$ (\Cref{fig:fig2}\fixme{f})
in 6 regions of interest (ROIs) (\Cref{fig:fig2}\fixme{e}).
...
(\Cref{fig:fig2}\fixme{b--d}),
which results in smaller time to threshold
(\Cref{fig:fig2}\fixme{f}).
...
resulting in $\sim 45\%$ (ROI 2) and \fixme{$\sim X\%$ (ROI 4)}
...
of the diencephalic (middle) ventricle (\Cref{fig:figure5_compare_cilia_modifications}a).
...
(\Cref{fig:figure5_compare_cilia_modifications}b).
...
(\Cref{fig:figure6_sim_results_compare_geometries}b).
