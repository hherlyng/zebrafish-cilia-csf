...
\title{Advection Versus Diffusion in Intraventricular Transport}
...
\keywords{cerebrospinal fluid, brain ventricles, fluid flow, transport}
...
including interactions with cerebrospinal fluid
(CSF). CSF is a clear liquid produced by the choroid plexus
that flows through the ventricular system and subarachnoid space.
By delivering growth factors, nutrients, neurotransmitters and metabolites
and removing waste products, CSF regulates brain development and homeostasis.
Disturbances of CSF circulation can lead to brain disorders, such as
hydrocephalus where accumulation of CSF often requires emergency surgical treatment. 

CSF movement occurs through multiple physiological processes,
including the action of motile cilia on the ventricular walls,
...
from 6 to 15 microns, they contribute to flow primarily at the surface
of the brain ventricles. In contrast, pressure gradients induce fluid
flow \fixme{further from the ventricular walls (bulk flow)} and along the
perivascular space surrounding the major blood vessels.

To date, it remains poorly understood how cilia contribute to the
overall fluid flow and solute transport in the brain.
...
cilia dynamics challenging to monitor. Until now, most studies have
...
gradients associated with respiration and heart pulsations in CSF
transport. Similarly, while MRI technology can measure CSF movement
...
movement and solute transport, and whether cilia dysfunction will
...
\fixme{potentially affecting brain activity and behavior.}
...
focused on modeling the interaction between single or several cilia and surrounding
...
the dynamics of a photoconvertible fluorescent protein Dendra2 expressed within the brain ventricles.
...
By imposing tangential traction and normal pressure on the CSF flow, we were able to reproduce
...
These results compared well with experimental data performed in vivo in zebrafish.
Then, by using both a computational model and zebrafish mutants with paralyzed cilia,
...
diffusion and advection, but also provide a novel framework to
...
Alternatively, it may relate to the way we defined the photoconversion site (ROI 1)
...
leak out of the brain ventricles and render the dynamics slower.
...
we manipulated certain parameters in our models. First, we paralyzed cilia in specific regions of
our model (\Cref{fig:fig5}). We identified that the cilia in the
dorsal middle ventricle contribute the most to solute distribution, which aligns well with the fact
that these cilia generate the strongest flow field in vivo~\cite{Olstad2019CiliaryDevelopment} and in our model.
...
Altogether, our model is flexible and thereby allows in silico testing of
...
CSF dynamics in more complex geometries \fixme{and other animal species and humans}.
...
relation~(\Cref{eq:D_stokes_einstein})~\cite{Einstein1905UberTeilchen}.
...
we also consider four variations in the geometry \fixme{(cross-sectional area
...
\subsection{Flow induced by cardiac pulsations}
...
advection-diffusion equation were implemented and solved numerically with DOLFINx~\cite{dolfinx2023preprint},
...
\subsection{Generation of transgenic line to study CSF \fixme{and transport} dynamics} %\emph{Tg(5xuas:Signal-Dendra2)\textsuperscript{nw21Tg}}
...
\begin{table}[H]
    ...
    \begin{tabular}{lcc}
        ...
        \midrule 
        Anterior region, anterior ventricle  & $-(1, 0, 1)$ & $\frac{2}{5}$ \\
        Posterior region, anterior ventricle & $\ \ \ (1, 0, 1)$ & 1 \\
        Dorsal region, middle ventricle      & $-(1, 0, 1)$ & $\lambda(\xx)=\frac{11}{4}\left(\frac{x-x_{0, d}}{x_{e, d} - x_{0, d}}\right)$\\
        Ventral region, middle ventricle     & $\ \ \ (1, 0, 1)$ & $\lambda(\xx) = \frac{1}{2}\left(1 - \frac{x-x_{0, v}}{x_{e, v} - x_{0, v}}\right)$ \\
        \bottomrule
    \end{tabular}
\end{table}
\endgroup
...
\begin{equation}
    \begin{aligned}
    \intO{\rho\pdifft{\uu}\cdot\vv} + \intO{2\mu\beps(\uu) : \beps(\vv)} - \intO{p\nabla\cdot\vv} &-\intGp{\mu(\nabla\uu)^T\nn\cdot\vv} \\ &=\intGp{\Tilde{p}(t)(\vv\cdot\nn)}, \quad\forall\vv\in\VV.
    \end{aligned}
...
\end{align*}
to the discrete weak  formulation~\eqref{eq:no_stabilization_abstract_discrete_weak_form_modelC},
...
\end{subequations}%
We solved~\eqref{eq:abstract_semi_discrete_weak_form_modelC}
numerically with the penalty parameter $\gamma=10$.

In time, we discretize the weak formulation~\eqref{eq:abstract_semi_discrete_weak_form_modelC}
...
Mean velocities $\overline{u}_x$~(\Crefpanels{fig:particle_tracking_data}{a}) and $\overline{u}_y$~(\Crefpanels{fig:particle_tracking_data}{b}),
...
\begin{figure}[H]
    ...
    \end{minipage} 
    \caption{Particle tracking data. Mean velocities in the $x$ and $y$ directions of the particles tracked. The dashed horizontal line designates a value of zero.}\label{fig:particle_tracking_data}
\end{figure}
