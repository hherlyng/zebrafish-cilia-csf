...
\maketitle
%
% Author: Please give full first and last names for authors and
% include * after the name of all corresponding authors
...
% Affiliations: Please provide adacemic titles (Prof. or Dr.) for all authors where applicable,
% and include an institutional email address for all corresponding authors
\begin{affiliations}
...
\keywords{cerebrospinal fluid, brain ventricles, cilia, transport, finite element methods}

% Abstract should be written in the present tense and impersonal style (i.e., avoid we), and be at most 200 words long
\begin{abstract}

Cerebrospinal fluid (CSF) flow is integral to brain function.
The underlying mechanisms of cilia-mediated flow and transport
in the brain ventricles remain to be fully explored.
In this paper, we analyze and evaluate the role of motile cilia
in intraventricular CSF flow and transport. We combine computational
modeling with in vivo experiments that monitor transport of a photoconvertible
protein secreted in the brain ventricles.
We use the geometry of the zebrafish larval brain ventricles,
for which we have detailed knowledge of cilia properties and CSF motion velocities and frequencies,
...
Altogether, our work offers both a computational framework that can be applied to other
ventricular system and new concepts of how molecules are transported within the brain and its ventricles.

\end{abstract}

\endgroup % End 0parindent group
...
(CSF)~\cite{CITATTION}. Produced mostly by the choroid
...
space~\cite{Damkier2013, Brodal2016}. By transporting growth factors, nutrients, neurotransmitters
...
hydrocephalus is observed in animal models and humans with deficient motile
cilia~\cite{Afzelius2004Cilia-relatedDiseases, Olstad2019CiliaryDevelopment, Mitchison2017Motile,
Ibanez-Tallon2002LossHydrocephalus},
...
Motile cilia are hair-like structures that line the ventricular surface of the brain,
known as the ependyma~\cite{mitchell2007evolution, Ringers2020Role, spassky2013motile}. 
The cilia beat in a coordinated pattern at frequencies of
10--40 Hz to mediate fluid flow~\cite{Ringers2023NovelEpithelia, roth2025structure}. 
...
%
%Until now, most studies have analyzed one physical mechanism at a
%time, especially in mammals.
%
%  Studies on animal models such as mice and
% zebrafish have provided unequivocal evidence that cilia are crucial
% for CSF flow in these animals~\cite{Olstad2019CiliaryDevelopment, Faubel2016Cilia}.
% It is clear that cilia affects advective intraventricular transport 
% to some extent. However a quantification of this effect, as well as of to which
% extent diffusion plays a role in transporting solutes, remains
% yet to be performed.
...
however, they failed to identify the role of pressure
...
can measure CSF movement in humans~\cite{feinberg1987human, battal2011cerebrospinal,
yamada2015current}, it cannot detect the microscopic
movements of cilia to understand their impact in the large human
brain~\cite{Siyahhan2014FlowVentricles}. Consequently, we lack a comprehensive understanding of how
cilia contribute to CSF motion and solute transport, and whether cilia
dysfunction alters CSF composition and thus leads to brain disorders.
...
We also found that the location of cilia, as well as the ventricular geometry, largely
impact solute distribution. Altogether, our work presents a computational
framework together with new concepts of how cilia distribute molecules and
...
% Why? (Intersting point #1. However, we do not know aspect #2. 
% How? (We therefore ...)
% What ... 

In-vivo particle tracking reveals characteristic CSF flow patterns in
zebrafish larval brain ventricles
(\Crefpanels{fig:fig1}{a--c})~\cite{Olstad2019CiliaryDevelopment}. We
...
(\Crefpanels{fig:fig1}{d}). Simulations show that this traction alone
...
speeds ($26.5$ \textmu m/s) occur in the vicinity of the dorsal cilia
...
boundaries (\Crefpanels{fig:fig1}{d, \fixme{g}}). The resulting flow
is pulsatile and directional (\Crefpanels{fig:fig1}{\fixme{f}}), and
...
rostrocaudal axis, reaching up to 27.7 \textmu m/s dorsally in the
...
\begin{figure}
    ...
    velocity magnitudes in the middle ventricle.
    ...
    %t=0.112613 s %t=0.225225 s %t=0.315315 s
...
%% Over one cardiac cycle, the CSF flow speed reaches up to
%% 27.7 \textmu m/s, dorsally in the posterior part of the middle
%% ventricle. This peak is attained at the time instant coinciding with
%% the time of peak caudal cardiac pulsatile flow, 

%% Using the observed maximum velocity magnitude and choosing the height
%% of the middle ventricle ($110$ \textmu m) as a characteristic length
%% scale yields a Reynolds number $\mathrm{Re} \approx 0.004$, justifying
%% the assumption of Stokes flow.


%We model the compartmentalized and pulsatile CSF flow observed in vivo~(\Crefpanels{fig:fig1}{b, c}) by the Stokes equations, and solve the equations numerically using a finite element method.

%To characterize CSF flow patterns predicted by
%computational fluid dynamics within the brain ventricles,

%we first consider the simulated CSF velocity and pressure when only
%modeling cilia motion, disregarding cardiac pressure pulsations.
...
different solutes injected within a region of interest (ROI1) in the
...
%As expected, transport occurs more rapidly with increasing diffusion
%coefficient.
...
$\mathrm{Pe}_1=519$, $\mathrm{Pe}_2=14.7$ and $\mathrm{Pe}_3=7.4$ for
$D_1$, $D_2$ and $D_3$, respectively. With all $\mathrm{Pe} > 1$, this
indicates that advection is the dominant transport mechanism on a
global scale.

%% To quantify the balance of advection versus diffusion as transport
%% mechanisms, we calculate the Péclet numbers $\mathrm{Pe}_1=519$,
%% $\mathrm{Pe}_2=14.7$ and $\mathrm{Pe}_3=7.4$ for $D_1$, $D_2$ and
%% $D_3$, respectively.  Since $\mathrm{Pe}>1$, this indicates that
%% advection is the dominant transport mechanism on a global scale.
...
ventricle (ROI5) and finally reaching the posterior ventricle
(ROI6). Notably, the extracellular vesicles, associated with the smallest diffusion
coefficient, do not reach the posterior ventricle (ROI6) within the
...
profiles in ROIs 1--6 (\Crefpanels{fig:fig3}{a, b}) show that the
...
the final time (0.72, 0.82, 0.56 and 0.35 in ROIs 2--6, respectively)
...
%% the $\cbar$ profiles of ROIs 3--4 are opposite order in the
%% simulations as compared to the experiments.

%% Notwithstanding similar dynamics, we observe small differences between
%% the experiments and simulation.

%% For instance, transport in ROIs 2--6 is faster in the simulation and
%% reaches higher final values.

%% We obtained very similar kinetics curves (\Crefpanels{fig:fig3}{a, b})
%% and time-to-threshold values (\Crefpanels{fig:fig3}{c, d}) when
%% comparing our experiments (\Crefpanels{fig:fig3}{a, c}) with
%% simulation (\Crefpanels{fig:fig3}{b, d}).

%% Importantly, since the initial condition of the simulation is a mean
%% concentration of zero, the mean concentration $\cbar$ can be
%% interpreted as the change in mean concentration and is thus a similar
%% metric to $\Delta F$.
...
\mer{Sentence on why absence of motile cilia is intersting: Cilia
  motion is key to development and absence of cilia is associated with
  pathologies.} As observed both experimentally and computationally,
the absence of motile cilia alters the CSF flow patterns. We next
investigate the implications of these changes on intraventricular
transport.
...
of fluorescence intensity (\Crefpanels{fig:fig4}{a}), resulting in
$45\%$ (ROI 2) and $25\%$ (ROI 4) higher mean times-to-threshold
(\Crefpanels{fig:fig4}{b}), in the \emph{smh} mutants compared to
controls. We found no significant differences in the Dendra2
distribution in ROIs 3, 5, and 6 between \emph{smh} mutants and
controls.
...
solute movement to regions upstream in the cilia flow fields or to
...
of the diencephalic (middle) ventricle (\Crefpanels{fig:fig5}{a}).
...
force~(\Crefpanels{fig:fig5}{b}). The magnitude of the force contributions are thus
...
in the diencephalic ventricle. The mean concentration profiles 
of all ROIs (except ROI 1, where we impose the photoconversion curve)
...
Meanwhile, removal of telencephalic cilia does not alter the 
...
and the combination of the above three modifications (\Crefpanels{fig:fig6}{a, b}). In these
...
(\Crefpanels{fig:fig6}{c}, shrunk fore-mid scenario). This is notwithstanding a
...
mean concentrations. Altogether, our results show that the geometry and
width of the ventricular ducts matter
...
\Cref{fig:fig4}, as the model and the fish have slightly different duct diameters.
\begin{figure}[H]
    \centering
...
other ventricular systems in the future, once the flow parameters of such systems
...
final simulation time (around 860 s), a ventricle where transport is largely driven
...
Advective transport by the mean velocity $\overline{U}=2.4 \ \mu$m/s over the same
distance would happen on a timescale of the order of $t_a=L/\overline{U}=83$ s.
...
exerted tangentially on the boundary, as opposed to a
couple of previous continuum model simulation studies of
cilia-mediated flow where cilia were represented by body
forces,~\cite{Siyahhan2014FlowVentricles, Thouvenin2020OriginCanal}.
We employed slip boundary conditions at the ventricular walls,
...
The BDM-DG scheme ($\mathrm{BDM}_1$-$\mathrm{DG}_0$) that we employed for the Stokes equations
...
as the elements by definition satisfy the incompressibility constraint for the fluid.
...
For each sample well, the \fixme{master} mix contained: 5 \textmu L of master mix; 0.14
...
downsampled by a block average using a resampling factor of 6. % (using the MATLAB function blockproc).
...
%To recover the flow fields shown in~\Crefpanels{fig:fig1}{b, c},
...
local refinement functionality in DOLFINx~\cite{dolfinx2023preprint}. The mesh
consists of 141 512 tetrahedral cells with a maximal (minimal) edge length of 14.9 \textmu
...
\begin{itemize}
    ...
    Here, we apply a tangential traction $\btau$ on the cilia boundary $\Gc$.
    In the absence of cardiac flow, the anterior/posterior boundary $\Gp=\emptyset$. % Model I/Model A
...
%% comparing these experimental velocity values
%% with the simulated, maximum velocity magnitude.

%% Both $\rr$ and the value $\tau=0.65$ mPa were chosen by 
%% calibration against experimentally measured flow speeds~\cite{Olstad2019CiliaryDevelopment}.
...
$U_r = 27.7$ \textmu m/s (the maximum velocity magnitude)
...
conservation properties. In time, the equation is discretized with the second-order backward
...
%\medskip
%\textbf{Supporting Information} \par %Please delete the Suppporting Information statement
%if it is not applicable. Please supply Supporting Information in another file.
%Supporting information should not be provided in .tex format
...
where we impose the free-slip condition~\Cref{eq:modelA_full_slip_BC}.
...
whereas~\Cref{eq:modelA_slip_BC_shear} imposes a stress tangential to
...
The boundary conditions~\Cref{eq:modelA_slip_BC} are set weakly through the stress
boundary integral 
\begin{equation}
    \intG{\bsig\nn\cdot\vv} =  \intG{\hat{\bsig}\cdot\vv}
    \label{eq:boundary_integral_BC1}
\end{equation}
in the weak form~\Cref{eq:modelA_weak_form1} of the Stokes equations.
...
\end{equation}
The second term on the right-hand side of~\Cref{eq:boundary_integral_BC2}
...
\end{equation}
Since $\Gp=\emptyset$ in this model version, and the term~\Cref{eq:boundary_integral_BC_normal}
...
Hence the system~\Cref{eq:stokes_weak_modelA} is singular with a one-dimensional nullspace
of constant pressures.
Consequently, assembly of our discretization of the  equations~\Cref{eq:stokes_weak_modelA}
...
such that the stress boundary integral~\Cref{eq:boundary_integral_BC1}
...
the boundary condition~\Cref{eq:modelB_pressure_BC},
...
formulation~\Cref{eq:abstract_weak_form_modelC,
eq:stokes_bilinear_form,eq:stokes_skew_form,eq:stokes_linear_form}
...
\end{subequations}%
We solved~\Cref{eq:abstract_semi_discrete_weak_form_modelC}
...
variables in~\Cref{eq:adv_diff_weak_formB} with their discrete
...
\begin{equation}
    \begin{aligned}
        ...
        +\sum_{f\in \mathcal{F}}\int_f{\jump{c_h \hat{u}_h}\cdot\jump{\phi_h}}\,\mathrm ds  \\
...
of \Cref{eq:SIP_stabilization} penalize jumps in the concentration flux across interior facets.
...
\end{equation}
In this work, we solved~\Cref{eq:discrete_weak_form_adv_diff} numerically
with the penalty parameter value $\alpha=25$.

In time, we discretized~\Cref{eq:discrete_weak_form_adv_diff} with the implicit Euler method.
...
Since the velocity field~\Cref{eq:velocity_mms} is divergence free by construction,
