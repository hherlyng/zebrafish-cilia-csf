...
covering a distance of at least 8.8 $\mu$m, were plotted in~\Cref{fig:fig1}b, c.
...
We seek to model the compartimentalized and pulsatile CSF flow observed \emph{in vivo}~(\Cref{fig:fig1}b--c).
...
flow is pulsatile as a result of the sinusoidal pressure gradient (\Cref{fig:fig1}g),
...
and directional flow in the posterior ventricle (\Cref{fig:fig1}e,
...
It has been observed, both previously and in this work, that the fluid velocity increases towards the wall~(\Cref{fig:fig1}c).
...
errors are expected to decay with order $k$, see for example Cockburn~\emph{et al.}~\cite{Cockburn2005AEquations}.
