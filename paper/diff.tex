...
%DIF DEL main_v1.tex   Wed Jun 25 13:39:44 2025
%DIF ADD main.tex      Wed Jun 25 17:32:29 2025
...
\begin{abstract}
Cerebrospinal fluid (CSF) is integral to brain function. CSF
provides mechanical support for the brain and helps
distribute nutrients, neurotransmitters and metabolites throughout the
central nervous system. CSF flow is driven by several processes,
including the beating of motile cilia located on the
walls of the brain ventricles. Despite the physiological importance of
CSF, the underlying mechanisms of CSF flow and solute transport
in the brain ventricles remain to be comprehensively
resolved. This study analyzes and evaluates specifically
the role of motile cilia in CSF flow and transport. 
We developed finite element methods for modeling flow
and transport using the geometry of embryonic zebrafish
brain ventricles, for which we have detailed knowledge of
cilia properties and CSF motion.
The computational model is validated by in vivo experiments
...
(CSF)~\cite{fame2020emergence}. Produced predominantly by the choroid
plexus, CSF is a clear liquid that flows through the ventricular system and subarachnoid
space~\cite{Brodal2016, Damkier2013, Wichmann2022Overview}.
By transporting growth factors, nutrients, neurotransmitters
...
Disruption of CSF circulation is detrimental to brain health and disturbs
cognitive and motor functions~\cite{Johanson2008},
and accumulation of CSF leads to hydrocephalus,
a disorder often requiring emergency surgical treatment.
\cite{duy2022rethinking, kahle2024paediatric, wallmeier2022role,
flannery2014pediatric}

The flow of CSF in the ventricular system is driven by multiple
physiological processes, including the beating motion of motile
...
macaulay2022cerebrospinal, mestre2018flow, Williams2023NeuralActivity}. 
...
Thouvenin2020OriginCanal}. Notably, motile cilia
are found on the ventricular surface of the brain,
known as the ependyma, and beat in a
...
primarily to flow near the ependyma in the
...
pulsatile flow at larger scales in the brain ventricular system,
subarachnoid space, and perivascular
...
as compared to other slower fluid dynamic processes that span larger
spatial scales~\cite{Vinje2019RespiratoryMeasurements, eide2024functional, fultz2019coupled, mestre2018flow},
make cilia difficult to monitor in vivo in the rodent and human brain.
...
and other applications such as ciliary transport of mucous and particles in
...
solute concentration dynamics. A geometry representing the embryonic zebrafish brain ventricles
...
cilia lining the surface of the brain ventricles\DIFaddbegin \DIFadd{, using a
computational geometry generated with confocal imaging data of
a zebrafish embryo (}\Crefpanels{fig:fig1}{d}\DIFadd{).
A description of the geometry is provided in }\Cref{sec:image_based_geometry}\DIFaddend .
...
(\Crefpanels{fig:fig1}{d}), with an impermeability condition (no normal flow) at the walls.
...
posterior ventricle (\Crefpanels{fig:fig1}{h--j},
\href{https://doi.org/10.5281/zenodo.15194757}{Supplementary Video S1}).
...
\begin{figure}
    ...
    \DIFaddbeginFL \DIFaddFL{We remark that the velocities in the cardiac-induced/no-cilia
    flow model are one order of magnitude lower (maximum 4.8 }\textmu \DIFaddFL{m/s) than the
    velocities in the flow models that include motile cilia.
    }\DIFaddendFL Scale bars 50 \textmu m.
    }
    \label{fig:fig1}
\end{figure}
...
dorsal region of the middle ventricle (\Crefpanels{fig:fig2}{a}) and
...
CSF flow structures are evident in the concentration field,
...
diffusion coefficient as solute spreads more uniformly throughout
...
to distal regions. Computing Péclet numbers, we find 
...
experiments in transgenic zebrafish embryos expressing the secreted
...
protein (\href{https://doi.org/10.5281/zenodo.15194757}{Supplementary Video S2}).
To quantify the transport of photoconverted proteins, we
...
\Crefpanels{fig:fig1}{g}~\cite{Olstad2019CiliaryDevelopment}. We
...
motile-cilia mutant \emph{schmalhans} (\emph{smh}, \emph{ccdc103}) zebrafish\cite{Panizzi2012Mutations}.
In ROIs 2 and 4, which quantify Dendra2 distribution downstream of the
...
the embryonic zebrafish brain~\cite{Olstad2019CiliaryDevelopment,
...
(\Crefpanels{fig:fig5}{c}). Finally, when removing ventral cilia in the
...
\begin{figure}[!ht]
    ...
    by removing the cilia tangential traction boundary condition in the respective region.
    \textbf{b.} The total force applied to the boundary with the cilia traction
    in each of the three separate ciliated regions, and the total force
    ...
    paralysis scenarios. The legend refers to the region where cilia are removed.}
    \label{fig:fig5}
\end{figure}
...
CSF velocities between embryos at given developmental stages and throughout
...
the original geometry (\Crefpanels{fig:fig6}{d}).
...
compared to the differences in experimental data in \Cref{fig:fig4},
...
solute transport within brain ventricles.  We used the embryonic
...
zebrafish embryos (\emph{smh}) aligned with these observations.
...
flow fields in embryonic zebrafish brain ventricles in which
...
\subsection{\DIFdelbegin \DIFdel{Numerical }\DIFdelend \DIFaddbegin \DIFadd{Limitations and numerical }\DIFaddend considerations}
\DIFaddbegin \DIFadd{Since we focused our study on the effect of cilia, we neglected
physiological effects other than cilia and cardiac pulsations on ventricular flow.
The approach used to model cardiac pulsations is simplified for several reasons.
One is the knowledge of the physiological processes involved in cardiac pulsations,
such as blood vessel contraction and dilation, brain tissue displacement
and interstitial fluid dynamics.
The description of these physiological processes in
embryonic zebrafish brain ventricles
is not as extensive as that for the human brain. We therefore
neglected wall displacements, blood vessel movement and fluid
displacement across the ventricular walls.
Second, our study considers a a younger brain that does not have a fully developed ventricular,
cardiovascular and nervous system.
This would however be highly relevant to model in more developed zebrafish,
where we expect that the above mentioned physiological effects will be more prominent. 
Third, availability of particle tracking data allowed for calibrating 
our cardiac pulsations model, since there is only one parameter, the normal
pressure amplitude, that needs to be determined.
}

\DIFadd{The simplified approach to modeling cardiac pulsations
resulted in CSF flow in and out of the 
anterior/posterior ends of the ventricles.
}\DIFaddend When simulating solute transport, we handled the advective transport
through \DIFdelbegin \DIFdel{the anterior/posterior boundaries
(where cardiac pressure pulsations were imposed)
}\DIFdelend \DIFaddbegin \DIFadd{these boundaries
}\DIFaddend via an approximate periodic boundary condition\DIFdelbegin \DIFdel{that conserved the }\DIFdelend \DIFaddbegin \DIFadd{.
This condition lets solute exit at the anterior or posterior
boundary and re-enter at the other side to
conserve the }\DIFaddend total solute concentration\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{,
to reflect our assumption that the embryonic zebrafish
ventricular system is closed at 2 dpf. This boundary
condition may influence the second-order accuracy of
the advection-diffusion equation discretization. 
To ensure that the model implementation did not
introduce excessive numerical diffusion, we
performed a refinement study of the transport model (see~}\Cref{subsec:numerical_verification_transport}\DIFadd{).
}\DIFaddend A possible improvement \DIFaddbegin \DIFadd{of the approximate periodic boundary
condition }\DIFaddend would be
...
Alternatively, the beating cardiac motion could be
modeled by \DIFdelbegin \DIFdel{ventricular wall deformation }\DIFdelend \DIFaddbegin \DIFadd{the above mentioned ventricular wall deformations
}\DIFaddend to avoid advective boundary fluxes\cite{Causemann2022HumanFramework,
Kurtcuoglu2005ComputationalSystem, Kurtcuoglu2007ComputationalSylvius, Linninger2005PulsatileBrain}\DIFdelbegin \DIFdel{.
%DIF < 
We also remark that the }\DIFdelend \DIFaddbegin \DIFadd{,
in the case that a quantitative description of such wall deformations for the embryonic
zebrafish brain ventricles were present.
}

\DIFadd{The }\DIFaddend Brezzi-Douglas-Marini and discontinuous Galerkin scheme
$\mathrm{BDM}_1$-$\mathrm{DG}_0$
used to discretize the Stokes equations is \DIFaddbegin \DIFadd{a first-order
approximation. Because of consistent results under a refinement
study (see~}\Cref{subsec:numerical_verification}\DIFadd{)
as well as qualitative replication of experimentally
observed flow fields, we deemed the approximation satisfactory
for the purposes of this study.
We also remark that the $\mathrm{BDM}_1$-$\mathrm{DG}_0$ scheme
is }\DIFaddend computationally more expensive
than more common discretizations such as 
...
the CSF velocity $\uu_h$ satisfies the continuity equation (\Cref{eq:stokes_eqs_cont}) point-wise, an important property for the stability of the advection-diffusion
...
directly~\cite{Brezzi1985TwoProblems} (see \Cref{subsec:appendixA1} for more details).

\subsection{Outlook}
Altogether, we have shown that the flow
generated by ciliary motion in embryonic zebrafish brain ventricles
is of the scale of the geometry~\cite{Olstad2019CiliaryDevelopment}.
In contrast, one can expect that in large brains such as the
human brain, ciliary beating contributes only to flow proximal
to the ventricular walls\cite{Siyahhan2014FlowVentricles}.
Nevertheless, the cilia may still play an important role in regulating the
amount of substances, especially extracellular vesicles,
close to the ventricular walls,
...
Notably, the discovery of a cilia-driven flow network
in rodent brains strongly supports such selective
brain ventricular transport~\cite{Faubel2016Cilia-basedVentricles}.
Our results also revealed that solute concentrations are affected
by ventricular geometry changes.
In fact, a 9\% difference in volume
had a noticeable impact on solute transport,
suggesting that pathological ventricular volume changes,
such as the enlargement observed in hydrocephalus,
could affect CSF distribution and composition.
...
The embryonic and adult zebrafish were raised under standard husbandry
...
From fertilization to 3 days post-fertilization (dpf), embryos were maintained in egg
...
experiments were performed with zebrafish embryos at the 2 dpf stage obtained from inbreeding of heterozygous
...
Two dpf embryonic zebrafish were anesthetized in 0.013\% MS222
...
openly available~\cite{Herlyng2025Zenodo}.  The
...
We carried out particle tracking in an embryonic zebrafish injected with fluorescent beads,
...
\subsection{Image-based computational geometries of zebrafish brain ventricles}\DIFaddbegin \label{sec:image_based_geometry}
\DIFaddend To model CSF flow and transport in the zebrafish brain ventricles, we
...
ventricular surfaces were marked \DIFdelbegin \DIFdel{to be }\DIFdelend \DIFaddbegin \DIFadd{as }\DIFaddend lined with motile cilia
(\Crefpanels{fig:fig1}{d}, magenta
markers)~\cite{Olstad2019CiliaryDevelopment}.
\DIFaddbegin \DIFadd{The surface geometry from confocal imaging data
and all computational meshes used in this work are openly
available~\mbox{%DIFAUXCMD
\cite{Herlyng2025Zenodo}}\hskip0pt%DIFAUXCMD
.
}\DIFaddend 
...
Particle tracking data \DIFdelbegin \DIFdel{were }\DIFdelend \DIFaddbegin \DIFadd{was }\DIFaddend used to determine the contributions of the
...
The \DIFdelbegin \DIFdel{ventricular walls }\DIFdelend \DIFaddbegin \DIFadd{ventricles }\DIFaddend are assumed to be \DIFdelbegin \DIFdel{impermeable (admitting }\emph{\DIFdel{no flux}}%DIFAUXCMD
\DIFdel{: $\JJ\cdot\nn=0$), except at the anterior/posterior boundary $\Gp$}\DIFdelend \DIFaddbegin \DIFadd{a closed system,
with impermeable ventricular walls. Therefore, we impose a no-flux condition
$\JJ\cdot\nn=0$ at the boundary $\Gamma\setminus\Gp$}\DIFaddend .
At the anterior/posterior boundary \DIFdelbegin \DIFdel{, we permit the solute concentration to be advected into
}\DIFdelend \DIFaddbegin \DIFadd{$\Gp$, solute concentration may be advected in }\DIFaddend and out
of the domain \DIFdelbegin \DIFdel{in a manner that ensures conservation of mass
(see}\DIFdelend \DIFaddbegin \DIFadd{by the CSF flow. To approximate the no-flux condition
on $\Gp$ and approximately achieve conservation of the total solute concentration,
we do the following. On the outflow part of $\Gp$ ($\uu\cdot\nn\geq 0$),
we impose zero diffusive flux ($D\nabla c\cdot\nn = 0$).
The total flux on the inflow part of $\Gp$ ($\uu\cdot\nn < 0$) is 
set equal to the total outflow flux at the previous timestep
to approximate a periodic boundary condition.
See}\DIFaddend ~\Cref{subsec:adv_diff_eq_weak_form} for \DIFdelbegin \DIFdel{details)}\DIFdelend \DIFaddbegin \DIFadd{further details}\DIFaddend .
...
The advection-diffusion equation (\Cref{eq:adv_diff_strong}) is discretized in
...
The numerical methods for the Stokes equations and the
advection-diffusion equation were implemented and solved
with DOLFINx\cite{dolfinx2023preprint}.
The simulation code is openly
available~\cite{Herlyng2025Zenodo}.
...
\noindent The software and data used and presented in this paper,
as well as the supplementary videos, are
archived on Zenodo and openly available at
\href{https://doi.org/10.5281/zenodo.15194757}{https://doi.org/10.5281/zenodo.15194757}\cite{Herlyng2025Zenodo}.
...
%%--------------APPENDIX--------------%%
\appendix
\section{Supplementary theory and numerical methods}
...
equations (\Cref{eq:appendix_stokes_eqs}), we consider the function space\footnote{
...
$t>0$. Similar convention is applied also when discussing the advection-diffusion equation in \Cref{subsec:adv_diff_eq_weak_form}.
}
\begin{align*}
...
the continuity equation (\Cref{eq:BDM_stokes_div}) is satisfied point-wise.
...
the discrete weak form (\Cref{eq:no_stabilization_abstract_discrete_weak_form_modelC}).
...
\Cref{eq:photoconversion_curve_appendix} is similar
...
\subsection{Numerical verification of \DIFaddbegin \DIFadd{CSF flow }\DIFaddend model implementation}\label{subsec:numerical_verification}
...
\Cref{eq:convergence_order_estimate}.
...
To verify the \DIFaddbegin \DIFadd{CSF flow }\DIFaddend model implementation with the zebrafish brain ventricles mesh,
...
fTetWild~\cite{Hu2020FastWild} \DIFdelbegin \DIFdel{; }\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend the region around the photoconversion region is not
refined on the mesh used for the convergence study\DIFaddbegin \DIFadd{), and we refined the original
mesh using fTetWild}\DIFaddend .
...
\DIFaddbegin \subsection{\DIFadd{Numerical verification of solute transport model implementation}}\label{subsec:numerical_verification_transport}
\DIFadd{To verify the solute transport model implementation with the zebrafish brain ventricles mesh,
we compared times to threshold and final total concentration values scaled by domain volume
for Dendra2 transport simulations with varying degrees of refinement of the brain ventricles
mesh~(}\Cref{tab:ventricles_norms_adv_diff}\DIFadd{).
We used the same meshes as the flow model verification (see }\Cref{subsec:numerical_verification}\DIFadd{).
The final total concentration values were scaled by $\vert\Omega\vert$ to account for the
fact that the domain changes slightly with mesh refinement.
}

\begin{table}[!htbp]
    \centering
    \small
    \caption{\DIFaddFL{Times to threshold in the six regions of interest
    and final total solute concentration
    $c_{\mathrm{tot}}(t=T) = \intO{c(\xx, T)}$
    scaled by domain volume $\vert\Omega\vert$
    under mesh refinement of the brain ventricles mesh, for simulations of Dendra2
    solute transport using the baseline model.}}
    \label{tab:ventricles_norms_adv_diff}
    \begin{tabular}{cc|ccccccc}
        \toprule
        \DIFaddFL{\# cells }& \DIFaddFL{\# dofs }&
        \DIFaddFL{$\hat{t}_1$ }[\DIFaddFL{s}] & \DIFaddFL{$\hat{t}_2$ }[\DIFaddFL{s}]& \DIFaddFL{$\hat{t}_3$ }[\DIFaddFL{s}]& \DIFaddFL{$\hat{t}_4$ }[\DIFaddFL{s}]& \DIFaddFL{$\hat{t}_5$ }[\DIFaddFL{s}]& 
        \DIFaddFL{$\hat{t}_6$ }[\DIFaddFL{s}] & \DIFaddFL{$\frac{1}{\vert\Omega\vert}c_{\mathrm{tot}}(t=T)$ }[\DIFaddFL{--}] \\
        \midrule 
        \DIFaddFL{132134  }& \DIFaddFL{1321340  }& \DIFaddFL{61 }& \DIFaddFL{199 }& \DIFaddFL{217 }& \DIFaddFL{167 }& \DIFaddFL{213 }& \DIFaddFL{342 }& \DIFaddFL{0.524 }\\
        \DIFaddFL{180901  }& \DIFaddFL{1809010  }& \DIFaddFL{61 }& \DIFaddFL{193 }& \DIFaddFL{210 }& \DIFaddFL{161 }& \DIFaddFL{208 }& \DIFaddFL{338 }& \DIFaddFL{0.532 }\\
        \DIFaddFL{262598  }& \DIFaddFL{2625980  }& \DIFaddFL{61 }& \DIFaddFL{190 }& \DIFaddFL{208 }& \DIFaddFL{160 }& \DIFaddFL{206 }& \DIFaddFL{336 }& \DIFaddFL{0.529 }\\
        \DIFaddFL{395339  }& \DIFaddFL{3953390  }& \DIFaddFL{61 }& \DIFaddFL{187 }& \DIFaddFL{205 }& \DIFaddFL{156 }& \DIFaddFL{204 }& \DIFaddFL{334 }& \DIFaddFL{0.543 }\\
        \DIFaddFL{627439  }& \DIFaddFL{6274390  }& \DIFaddFL{61 }& \DIFaddFL{184 }& \DIFaddFL{201 }& \DIFaddFL{152 }& \DIFaddFL{205 }& \DIFaddFL{326 }& \DIFaddFL{0.556 }\\
        \bottomrule
    \end{tabular}
\end{table}

\DIFaddend \subsection{Discussion of the numerical approximation and convergence rates}
...
and the pressure in the $L^2$ error norm on the cylinder mesh. \DIFdelbegin \DIFdel{For }\DIFdelend \DIFaddbegin \DIFadd{The BDM-DG scheme is
thus of first-order accuracy, which may introduce excessive numerical diffusion.
However, for }\DIFaddend the ventricles meshes, we observe consistent values of the reported quantities
\DIFaddbegin \DIFadd{(}\Cref{tab:ventricles_norms_BDM}\DIFadd{)}\DIFaddend , verifying the model implementation.

The \DIFaddbegin \DIFadd{advection-diffusion equation discretization used is of second-order accuracy in 
both space and time. This accuracy may be influenced by the approximate periodic
boundary condition that we employ on the anterior/posterior boundary.
To verify the implementation, we report times to threshold in the six regions of interest,
as well as the final total solute concentration (}\Cref{tab:ventricles_norms_adv_diff}\DIFadd{)
for varying degrees of refinement of the ventricles mesh.
We observe consistent values across the meshes.
Similar to the flow model being influenced by the slight changes
in the size of cilia-populated marked regions with mesh refinement,
owing to the unstructured organization of the mesh cells, the 
photoconversion region (region of interest 1) increases slightly
with mesh refinement. This is presumably the cause for
the increase in total final concentration and thus
shorter times to threshold.
}

\DIFadd{The }\DIFaddend $\mathrm{BDM}_1$-$\mathrm{DG}_0$ scheme that we employ for the Stokes equations
...
Particle tracking data\DIFdelbegin \DIFdel{was analyzed~\mbox{%DIFAUXCMD
\cite{Olstad2019CiliaryDevelopment} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{Olstad2019CiliaryDevelopment} }\hskip0pt%DIFAUXCMD
was analyzed
to discern the relative contribution of cardiac pulsations to CSF flow,
as compared to the contribution of motile cilia}\DIFaddend .
...
diencephalic ventricle\DIFaddbegin \DIFadd{, where cilia do not greatly impact CSF flow~\mbox{%DIFAUXCMD
\cite{Olstad2019CiliaryDevelopment}}\hskip0pt%DIFAUXCMD
}\DIFaddend .
Frequency of image acquisition was 20.22 Hz.
...
$\overline{u}_z$~(\Crefpanels{fig:particle_tracking_data}{b}),
...
\begin{figure}[H]
    ...
    along the rostrocaudal axis ($x$ axis, \textbf{a})
    and along the dorsoventral axis ($z$ axis, \textbf{b})
